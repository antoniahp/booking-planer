...
This repo contains all the code of the website from https:// traveling.com
...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   DEBUG=1
   SECRET_KEY=dsfdgrew45647ioikjh
   DJANGO_ALLOWED_HOSTS=localhost 127.0.0.1 [::1]
   SQL_ENGINE=django.db.backends.postgresql
   SQL_DATABASE= traveling   _dev
   SQL_USER=asdfsgdvszr3r5e
   SQL_PASSWORD=sedr436tryut6uijyfhdfg
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_USER=asdfsgdvszr3r5e
   POSTGRES_PASSWORD=sedr436tryut6uijyfhdfg
   POSTGRES_DB= traveling   _dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=traveling_postgres_data_dev
    $ docker volume create --name=traveling_grafana
    ```
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/traveling** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
...
It will automatically load the provisioning configurations and serve the dashboard in [http:// traveling.com:3000](http:// traveling.com:3000).
